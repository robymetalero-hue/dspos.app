...
// Find start line of old middleware
const start = code.findIndex(l => l.includes('// Audit User Context Extraction Middleware'));
// Find end line of old middleware
let end = -1;
for (let i = start + 1; i < code.length; i++) {
  if (code[i].includes('  // TRACEABILITY: Global API Modification Logger')) {
    end = i - 1;
    break;
  }
}
...
if (start !== -1 && end !== -1) {
  code.splice(start, end - start, ...newAuth);
